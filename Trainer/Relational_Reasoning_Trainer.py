...
class InterIntra_Train(torch.nn.Module):
    """
    InterIntra_Train class represents a module for training a model using inter-sample and intra-temporal reasoning.
    Args:
    ...
    Attributes:
        backbone (torch.nn.Module): The backbone model.
        Inter_sample_relation (torch.nn.Sequential): The inter-sample relation module.
        Intra_temporal_head (torch.nn.Sequential): The intra-temporal head module.
    Methods:
        aggregate(features, K): Aggregates the features and returns the relation pairs and targets.
        run_test(predict, labels): Runs the test and returns the number of correct predictions and the total number of labels.
        train(x_train, y_train, K, train_transform, cut_piece, tensor_transform, batch_size, graph_name, tot_epochs, opt, window_size): Trains the model.
    """
    ...
    def aggregate(self, features, K):
        ...
        for index_1 in range(0, size*K, size):
            ...
            for index_2 in range(index_1+size, size*K, size):
                ...
                targets_list.append(torch.ones(
                    size, dtype=torch.float32).cuda())
                targets_list.append(torch.zeros(
                    size, dtype=torch.float32).cuda())
    ...
    def train(self, x_train, y_train, K,
              train_transform, cut_piece,
              tensor_transform, batch_size, graph_name, tot_epochs, opt, window_size):
        ...
        train_set = LPBF_Inter_Intra_reasoning(data=x_train, targets=y_train, K=K,
                                               ...
                                               totensor_transform=tensor_transform, n_epoch=100, starting_p=0, stopping_p=window_size, window_size=window_size)
        ...
        num_steps_per_epoch = math.floor(
            len(train_loader.dataset) / opt.batch_size)
        ...
        scheduler = CosineAnnealingLR(
            optimizer, T_max=num_steps_per_epoch * tot_epochs, eta_min=1e-4)
        ...
        for epoch in range(tot_epochs):
            ...
            num = random.randint(1, window_size//2)
            print("Window_size", num)
            Times.append(num)

            len_org_data = len(x_train[1])
            # print("Have a loader ", len_org_data)

            starting_length = np.random.randint(0, num)  # Length of anchors
            # print("Starting_point:", starting_length)

            stopping_length = starting_length + \
                (window_size-num)  # Length of anchors
            # print("Stopping_length:", stopping_length)

            train_set = LPBF_Inter_Intra_reasoning(data=x_train, targets=y_train, K=K,
                                                   transform=train_transform, transform_cut=cut_piece,
                                                   totensor_transform=tensor_transform, n_epoch=num, starting_p=starting_length, stopping_p=stopping_length, window_size=window_size)
            ...
            # open the arena of looping across each loop
            epoch_smoothing = []
            for i, (data, data_augmented0, data_augmented1, data_label, _) in enumerate(train_loader):
                ...
                # print("shape of the data before the network", x.shape)
                ...
                # print("find shape features", features.shape)
                features_cut0 = self.backbone(x_cut0)
                # print("find shape features_cut0", features_cut0.shape)
                features_cut1 = self.backbone(x_cut1)
                # print("find shape features_cut1", features_cut1.shape)
            ...
            if (epoch+1) % tot_epochs == 0:
                ...
                torch.save(self.backbone.state_dict(),
                           '{}/backbone_{}.tar'.format(folder_created, tot_epochs))
        ...
        return Training_accuracy, Training_loss, Training_loss_mean, Training_loss_std, learn_rate, Times
