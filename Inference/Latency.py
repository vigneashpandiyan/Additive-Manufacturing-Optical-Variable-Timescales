# -*- coding: utf-8 -*-
"""
Created on Sun Jun  8 19:07:53 2025
...
def process_loader(loader, backbone_lineval, folder_created, filename, epoch_length, name):
    ...
    print(f"{name.capitalize()} processing time per sample: {mean_time:.6f} ± {std_time:.6f} sec/sample")
...
def Inference_calc(x_train, y_train, x_val, y_val, x_test, y_test, ckpt, opt, folder_created, filename, epoch_length):
    ...
    train_set = LPBF_loader_eval(x_train, y_train, transform_lineval, n_epoch=epoch_length)
    val_set = LPBF_loader_eval(x_val, y_val, transform_lineval, n_epoch=epoch_length)
    test_set = LPBF_loader_eval(x_test, y_test, transform_lineval, n_epoch=epoch_length)

    train_loader = torch.utils.data.DataLoader(train_set, batch_size=128, shuffle=True)
    val_loader = torch.utils.data.DataLoader(val_set, batch_size=128, shuffle=False)
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=128, shuffle=False)

    val_emb, val_lab, val_mean, val_std = process_loader(val_loader, backbone_lineval, folder_created, filename, epoch_length, "val")
    test_emb, test_lab, test_mean, test_std = process_loader(test_loader, backbone_lineval, folder_created, filename, epoch_length, "test")
    train_emb, train_lab, train_mean, train_std = process_loader(train_loader, backbone_lineval, folder_created, filename, epoch_length, "train")
