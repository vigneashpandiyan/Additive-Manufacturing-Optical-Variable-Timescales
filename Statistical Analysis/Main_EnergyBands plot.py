...
def boxcomparisonplots(y_pred, y_true, path):
    ...
    Energybands.columns = ['Frequency distribution',
                           'Frequency levels', 'Categorical']

    plt.rcParams.update(plt.rcParamsDefault)
    plt.figure(figsize=(5, 3))
    sns.set(font_scale=2)
    sns.set_style("whitegrid", {'axes.grid': False})
    ax = sns.catplot(y="Frequency levels", x="Frequency distribution", hue="Categorical", kind="bar", data=Energybands, height=15,
                     aspect=1.5, palette=color)

    # Ensure the x-axis is in scientific notation
    # ax.set(xscale="linear")  # Ensure it's set to linear scale first
    ax.ax.xaxis.set_major_formatter(ScalarFormatter(useMathText=True))
    ax.ax.xaxis.get_major_formatter().set_scientific(True)
    ax.ax.xaxis.get_major_formatter().set_powerlimits((-4, -4))
    ...
    plt.savefig(os.path.join(path_, plotname), dpi=400, bbox_inches='tight')
    plt.show()
...
y_pred = Featurespace
y_true = classspace
Energybands = boxcomparisonplots(y_pred, y_true, total_path)
