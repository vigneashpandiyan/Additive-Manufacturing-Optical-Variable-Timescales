...
def latent_plots(folder_created, graph_name, epoch_length):
    ...
    train_labelsname = folder_created + '/' + \
        str(graph_name) + '_' + str(epoch_length) + '_labels' + '.npy'
    ...
    unique_numbers = df2['Categorical'].unique()

    label_mapping = {num: name for num, name in zip(unique_numbers, labels)}
    print(label_mapping)

    df2['Categorical'] = df2['Categorical'].map(label_mapping)
    # print(columns)
...
def plot_windows(epochs, window_size, path):
    """
    Plots a histogram of window lengths based on the given epochs and window size.
    Args:
    ...
    sns.histplot(window_length, color='blue', edgecolor="k", kde=True, bins=25,
                 line_kws={'color': 'crimson', 'linewidth': 5, 'linestyle': ':'})
    plt.ylabel("Relative frequency")
    plt.xlabel("Window lengths")
    plt.title("Epochs " + str(len(epochs)))
...
def distribution_plot(data, i, folder_created):
    ...
    Returns:
        None
    """
    labels_ = ['10 um', '20 um', '30 um', '40 um',
               '50 um', '60 um', '70 um', '80 um', '90 um', '100 um', '110 um']
    ...
    color = iter(cm.rainbow(np.linspace(0, 1, len(labels_))))
    ...
    for j in range(len(labels_)):
        ...
        data_1 = data[data.target == label_]
        data_1 = data_1.drop(labels='target', axis=1)
        c = next(color)
        data_1['Feature'] = (data_1['Feature'] - data_1['Feature'].min()) / (
            data_1['Feature'].max() - data_1['Feature'].min())
        fig = sns.kdeplot(data_1['Feature'], shade=True, alpha=.5, color=c)

    plt.title("Weight " + str(i+1))
    plt.legend(labels=labels_, bbox_to_anchor=(1.49, 1.05))
    title = folder_created+'/'+'Dimension'+'_' + \
        str(i+1)+'_'+'distribution_plot'+'.png'
    # plt.xlim([0.0, np.max(data)])
    # plt.ylim([0.0, 65])
    plt.xlabel('Weight distribution (a.u)')
...
def Cummulative_dist_plot(data, i, ax):
    ...
    ax.xaxis.set_major_locator(MaxNLocator(nbins=3))  # For X-axis
    ax.yaxis.set_major_locator(MaxNLocator(nbins=3))  # For Y-axis
    ax.set_title("Weight-" + str(i+1), y=1.0, pad=2, fontsize=12)
    ax.set_xlabel('Weight distribution (a.u)', fontsize=13)
