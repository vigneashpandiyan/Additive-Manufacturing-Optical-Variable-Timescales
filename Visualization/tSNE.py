...
def tsne_visualization(x_train, y_train, x_val, y_val, x_test, y_test, ckpt, opt, folder_created, filename, epoch_length):
    ...
    train_labelsname = folder_created+'/' + \
        str(filename)+'_'+str(epoch_length)+'_test_labels'+'.npy'
    np.save(train_embeddings, X, allow_pickle=True)
    np.save(train_labelsname, y, allow_pickle=True)

    graph_name1 = folder_created+'/' + \
        str(filename)+'_2D'+'_'+str(epoch_length)+'.png'
    graph_name2 = folder_created+'/' + \
        str(filename)+'_3D'+'_'+str(epoch_length)+'.png'
    graph_name3 = folder_created+'/' + \
        str(filename)+'_3D'+'_'+str(epoch_length)+'.gif'

    ax, fig, graph_name = TSNEplot(X, y, graph_name1, graph_name2,  graph_name3, str(
        filename), epoch_length, limits=2.6, perplexity=20)
