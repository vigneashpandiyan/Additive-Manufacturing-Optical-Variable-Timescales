...
# hlp.plot_d(x_train[0], t, jitter, "red",
#            save_file='Jittering.png', Transformation='Jittering')
# hlp.plot1d(x_train[0], aug.jitter(x_train)[0])

# Scaling
scaling = aug.scaling(x_train)[0]
# hlp.plot_d(x_train[0], t, scaling, "blue",
#            save_file='Scaling.png', Transformation='Scaling')
# hlp.plot1d(x_train[0], aug.scaling(x_train)[0])

# Rotation
rotation = aug.rotation(x_train)[0]
# hlp.plot_d(x_train[0], t, rotation, "green",
#            save_file='Rotation.png', Transformation='Rotation')
# hlp.plot1d(x_train[0], aug.rotation(x_train)[0])

# Cutout
cutout = aug.cutout((x_train)[0])
# hlp.plot_d(x_train[0], t, cutout, "pink",
#            save_file='Cutout.png', Transformation='Cut out')
...
funct = ['Original', 'Jitter', 'Scaling', 'Cutout',
         'Magnitude_warp', 'Time_warp', 'Window_slice', 'Window_warp']

# data_created = [(x_train)[0], jitter, scaling, rotation, cutout, permutation,
#                 magnitude_warp, time_warp, window_slice, window_warp]

data_created = [(x_train)[0], jitter, scaling, cutout,
                magnitude_warp, time_warp, window_slice, window_warp]
...
for i, key in enumerate(funct):
    ...
    if i == 0:
        ax.plot(t, y.ravel(), color='black', ls='solid',  linewidth=1)
        ...
        ax.legend(['Original signal'], fontsize=12)
...
# Assuming funct and data_created are already defined
n_plots = len(funct)
n_cols = 2  # Set the number of columns to 2
n_rows = (n_plots + 1) // n_cols  # Calculate the number of rows, rounding up

fig, axs = plt.subplots(
    nrows=n_rows,
    ncols=n_cols,
    sharey=False,
    figsize=(10, 12),  # Adjust the figsize as needed for better layout
    dpi=800
)

# Flatten the axs array to iterate over it easily, even if 2D
axs = axs.flatten()
...
for i, key in enumerate(funct):
    ...
    if i == 0:
        ...
        # Custom plotting for subsequent subplots
        hlp.plot_time_series(x, t, y, ax, colour[i], transformation)

# Remove any empty subplots if the number of plots is not a multiple of n_cols
if n_plots % n_cols != 0:
...
fig.tight_layout()
graphname = 'Augmentation_AE_signal.png'
plt.savefig(graphname, bbox_inches='tight', pad_inches=0.1)
plt.show()
